...
- **Semantic spine** (governs every color choice): warm = motion/action/caution, cool = structure/rest/data; within warm, redder = bites harder; danger rides on hue, never saturation. Full rationale + buildability constraints in `docs/superpowers/specs/2026-07-07-semantic-color-philosophy.md`. Two channels: hue = role/trust (recolor), underline = mutation (`EFFECT_TYPE 1`).
- Warm anchor: code functions & methods are terracotta `#E1B28E` (hue ~26°, the open warm slot past the peach hazard band — 7 tokens: the `DEFAULT_*` function/method tokens, `JS.INSTANCE_MEMBER_FUNCTION`, `JS.GLOBAL_FUNCTION`, `ReSharper.EXTENSION_METHOD_IDENTIFIER`). Was rose-gold `#E9BCAB` (16°) — moved out to un-blur it from params (see `docs/superpowers/specs/2026-07-09-method-param-distinction-terracotta.md`). Also mirrored in `ocean-harbor.theme.json` `"function"`. Blue `#89B4F7` is markup tags + CSS functions only. When touching function color, grep `E1B28E` and update all 7 (+ theme.json).
- Params = warm cream `#E4D9C6` (hue ~38°, boundary data — up-arc from the terracotta method, same warm family, calmer). 3 tokens: `DEFAULT_PARAMETER`, `JS.PARAMETER`, `DEFAULT_REASSIGNED_PARAMETER` (FG + EFFECT_COLOR). Was `#E6D8C0`; dropped saturation 43→36 to separate from method. Params no longer use cyan `A7DBD8` — grep `E4D9C6` to find them; leftover `A7DBD8` is rainbow brackets / annotations / format-strings, not params.
