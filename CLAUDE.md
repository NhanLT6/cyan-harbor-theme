...
- Living green layer: code functions & methods are kelp green `#7FDD9A` (hue ~137°, sat ~58% — bumped up from the initial `#93CDA1` so methods stand out against keyword purple / string terracotta — 7 tokens: the `DEFAULT_*` function/method tokens, `JS.INSTANCE_MEMBER_FUNCTION`, `JS.GLOBAL_FUNCTION`, `ReSharper.EXTENSION_METHOD_IDENTIFIER`). Was terracotta `#E1B28E` (retired 2026-07-13, that hex is now strings). Also mirrored in `ocean-harbor.theme.json` `"function"`. Blue `#89B4F7` is markup tags + CSS functions only. When touching function color, grep `7FDD9A` and update all 7 (+ theme.json).
